...
import { ICustomizationSettings } from '@/entities/Customization/types/types';
import { IObject } from '@daybrush/utils';
import { getAccurateAgent } from '@egjs/agent';
import { invert, matrix3d } from '@scena/matrix';
import Guides from '@scena/react-guides';

import { CustomizationDynamicInfoBar } from '../demoOld/ui/CustomizationDynamicInfoBar/CustomizationDynamicInfoBar';
import { DATA_SCENA_ELEMENT_ID, EDITOR_CSS, EditorContext, SESSION_STORAGE_ITEMS } from './consts';
...
> {
  private static screenType = window.localStorage.getItem(
    SESSION_STORAGE_ITEMS.CUSTOMIZATION_SCREEN,
  );
  private static returnScreenWidth = (screen: string): number => {
    if (screen === 'Desktop') {
      return this.url.includes('customization/post') ? 600 : 1025;
    } else if (screen === 'Tablet') {
      return this.url.includes('customization/post') ? 554 : 768;
    } else {
      return this.url.includes('customization/post') ? 514 : 465;
    }
    return this.url.includes('customization/post') ? 800 : 1025;
  };
  public static url = window.location.href;
  public static defaultProps = {
    width: this.returnScreenWidth(this.screenType || 'Desktop'),
    height: this.url.includes('customization/post') ? 400 : 844,
  };
  ...
  public handleBeforeUnload = () => {
    const savedTargets = this.saveTargets(this.selecto.current!.getSelectableElements());
    const savedSettings: ICustomizationSettings | null = JSON.parse(
      window.localStorage.getItem(SESSION_STORAGE_ITEMS.CUSTOMIZATION_SETTINGS) || '',
    );
    if (savedSettings) {
      if (!savedSettings.Tablet) {
        const savedTargetsTablet = savedTargets;
        savedTargetsTablet.map((item) => {
          if (item.frame.width > Editor.returnScreenWidth('Tablet')) {
            item.frame.width = Editor.returnScreenWidth('Tablet');
          }
        });
        window.localStorage.setItem(
          SESSION_STORAGE_ITEMS.CUSTOMIZATION_SAVED_JSON_TABLET,
          JSON.stringify(savedTargetsTablet),
        );
      }
      if (!savedSettings.Mobile) {
        const savedTargetsMobile = savedTargets;
        savedTargetsMobile.map((item) => {
          if (item.frame.width > Editor.returnScreenWidth('Mobile')) {
            item.frame.width = Editor.returnScreenWidth('Mobile');
          }
        });
        window.localStorage.setItem(
          SESSION_STORAGE_ITEMS.CUSTOMIZATION_SAVED_JSON_MOBILE,
          JSON.stringify(savedTargetsMobile),
        );
      }
    }
    window.localStorage.setItem(
  ...
  public render() {
    return <EditorContext.Provider value={this}>{this.renderChildren()}</EditorContext.Provider>;
  }
  ...
  public async componentDidMount() {
    ...
    // window.addEventListener('resize', this.handleBeforeUnload);
