import {
  ...
  TextTool,
  UndoTool,
} from '@/widgets/Customizator/Editor/tools';

import type { Tool, ToolGroup, ToolId } from '@/widgets/Customizator/Editor/types';

import type { ToolbarItem } from '@/widgets/Customizator/Editor/types';
...
export const tools: Partial<{ [id in ToolId]: Tool }> = {};
CropToolGroup.tools.forEach((tool) => {
  tools[tool.id] = tool;
});

// Create key-value pairs for ToolbarItems
(ToolbarItems.filter((item) => item.type === 'tool') as Tool[]).forEach((tool: Tool) => {
  tools[tool.id] = tool;
});
