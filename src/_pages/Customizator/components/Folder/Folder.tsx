...
import { between, prefix } from '@/_pages/Customizator/utils';
import { IObject, findIndex, hasClass, isArray, isObject } from '@daybrush/utils';
...
export default class Folder<T = any> extends PureComponent<{
  scope: string[];
  name: string;
  ...
  public folderRef = createRef<StyledElement<HTMLDivElement>>();
  public shadowRef = createRef<HTMLDivElement>();
  public state: {
    fold: boolean;
  ...
  public render() {
    ...
    return (
      <FolderElement className={prefix('folder')} ref={this.folderRef}>
        {name && (
          <div
  ...
  private onDragEnd = (e: OnDragEnd) => {
    ...
    const datas = e.datas;
    const prevInfo: FileInfo<T> | undefined = datas.prevInfo;
    ...
    //TODO: temporarily ignore nesting
    ...
    //TODO: do not allow nesting for any element
    if (parentInfo?.value) {
      const element = parentInfo.value as unknown as { jsx: { type: string } };
      if (element?.jsx.type === 'img') {
        /**
         * img tag can not have children, for further details, google:
    ...
    onMove(selectedInfos, undefined, isTop ? undefined : prevInfo);
    // if (!parentInfo && isTop) {
    //   onMove(selectedInfos);
    // } else if (parentInfo || prevInfo) {
    //TODO: temporarily disallow nesting elements
  ...
  private flatChildren(
    ...
    keys.forEach((key) => {
      ...
      if (
        valueChildren &&
        (isArray(valueChildren) ? valueChildren.length : isObject(valueChildren))
      ) {
